...
const addCommentCourse=asyncHandler(async(req,res)=>{
    const {name,email,comment,course_id}=req.body;
    const defaultAction = 'not approved'; // Replace with your default action value

    const sqlInsert = 'INSERT INTO commentcourse (name, email, comment, course_id, action) VALUES (?, ?, ?, ?, ?)';
    db.query(sqlInsert,[name,email,comment,course_id,defaultAction
...
const getCommentcourse = asyncHandler(async (req, res) => {
    // Fetch data from course and department tables to return department name
    const sqlSelect = `
